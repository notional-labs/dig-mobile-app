...
class ActiveAccountDetailPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final List<PieChartSeries> data = [
      PieChartSeries('Balance', 0.58, 2637.83, 0, const Color.fromRGBO(42, 157, 143, 1), AppAssets.icBalance),
      PieChartSeries('Delegation', 99.33, 4502, 0, const Color.fromRGBO(156, 183, 211, 1), AppAssets.icDelegation),
              ...
              children: [
                DSPrimaryAppBar.normal(
                 ...
                 Padding(
                  ...
                  child: Text('Assets', style: DSTextStyle.tsMontserratT20B
                      ...
                      child: SfCircularChart(
                          series: <CircularSeries>[
                            ...
                            DoughnutSeries<PieChartSeries, String>(
                                dataSource: data,
                                innerRadius: '70%',
                                pointColorMapper:(PieChartSeries data,  _) => data.color,
                                xValueMapper: (PieChartSeries data, _) => data.name,
                                yValueMapper: (PieChartSeries data, _) => data.digValue
                            )
                          ]
                  )
                )),
                ...
                const Align(
                  ...
                  child: Text('Total: 0\$', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold),),
                )
              ],
            ),
          ),
...
class PieChartSeries {
  ...
  PieChartSeries(this.name, this.percentage, this.digValue, this.value, this.color, this.image);
}

class AssetsDetailItem extends StatelessWidget {
  final PieChartSeries pieChartSeries;
  const AssetsDetailItem({required this.pieChartSeries, Key? key }) : super(key: key);
      ...
      child: Row(
        ...
        children: [
          ...
          Padding(
            ...
            child: Text(pieChartSeries.name, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold),),
          ),
          Padding(
            padding: const EdgeInsets.only(top: 2.0),
            child: Text('${pieChartSeries.percentage}%', style: const TextStyle(color: Colors.white)),
          ),
          const Spacer(),
          Padding(
            padding: const EdgeInsets.only(top: 2.0),
            child: Column(
              ...
              children: [
                Text('${pieChartSeries.digValue} DIG', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold),),
                Text('\$${pieChartSeries.value}', style: const TextStyle(color: Colors.white)),
              ],
            ),
          ),
        ],
